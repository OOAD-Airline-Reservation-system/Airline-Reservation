...
| Member | Use Case Owned | Files Owned | Design Principle | Design Pattern |
| --- | --- | --- | --- | --- |
| **Rohan** | Flight Search, Seat Selection & Booking + entire Database Layer | **Frontend:** `HomePage.jsx`, `SeatPage.jsx`, `ConfirmPage.jsx`, `PassengerFormPage.jsx`, `MyBookingsPage.jsx`, `AuthContext.jsx`, `BookingContext.jsx`, `ProtectedRoute.jsx`, `api.js`, `Navbar.jsx`, `firebaseConfig.js`, `vite.config.js`, `.env.local` **Controllers:** `FlightController.java`, `SeatController.java`, `BookingController.java` **Services:** `FlightService.java`, `SeatService.java`, `BookingService.java` **All 8 Repositories:** `FlightRepository.java`, `SeatRepository.java`, `BookingRepository.java`, `UserRepository.java`, `PaymentRepository.java`, `LoyaltyAccountRepository.java`, `LoyaltyTransactionRepository.java`, `PassengerDetailsRepository.java` **Config:** `FirestoreConfig.java`, `AppProperties.java`, `WebConfig.java` **Entities/DTOs:** `Flight.java`, `Seat.java`, `SeatClass.java`, `SeatStatus.java`, `Booking.java`, `BookingStatus.java`, `FlightResponse.java`, `SeatResponse.java`, `CreateBookingRequest.java`, `BookingResponse.java` | Pure Fabrication (GRASP) — All 8 repository classes are invented purely to handle Firestore persistence, representing no real-world airline concept, keeping persistence completely separate from business logic | Factory Method (Creational) — `BookingFactory` interface with `DefaultBookingFactory` centralises `Booking` object creation with reference generation, PENDING status, and timestamp in one place |
| **Aditya** | Payment Processing & External API Integration | **Frontend:** `PaymentPage.jsx`, `SuccessPage.jsx` **Controllers:** `PaymentController.java`, `PassengerDetailsController.java` **Services:** `PaymentService.java`, `PassengerDetailsService.java` **Gateways:** `PaymentGatewayAdapter.java`, `RazorpayGatewayAdapter.java`, `FlightTrackingClient.java`, `AviationstackClient.java`, `AeroDataBoxClient.java` **Factories:** `PaymentFactory.java`, `DefaultPaymentFactory.java` **Entities/DTOs:** `Payment.java`, `PaymentStatus.java`, `PassengerDetails.java`, `PaymentRequest.java`, `PaymentResponse.java` | Low Coupling (GRASP) — `PaymentService` depends entirely on the `PaymentGatewayAdapter` interface, never on `RazorpayGatewayAdapter` directly. Swapping payment providers requires zero changes to existing service logic | **Adapter (Structural)** — `RazorpayGatewayAdapter` adapts Razorpay SDK to `PaymentGatewayAdapter`; `AviationstackClient` adapts Aviationstack REST API to `FlightTrackingClient` **Strategy (Behavioral)** — `PaymentService` holds a `PaymentGatewayAdapter` reference injected at runtime, provider fully swappable **Factory Method (Creational)** — `PaymentFactory` with `DefaultPaymentFactory` centralises `Payment` object construction |
| **Member 3** | User Authentication & Security | **Frontend:** `LoginPage.jsx`, `RegisterPage.jsx` **Controllers:** `AuthController.java` **Services:** `AuthService.java` **Security:** `JwtService.java`, `JwtAuthenticationFilter.java`, `AppUserDetailsService.java` **Config:** `SecurityConfig.java` **Exceptions:** `GlobalExceptionHandler.java`, `BadRequestException.java`, `ResourceNotFoundException.java`, `UnauthorizedException.java`, `ApiErrorResponse.java` **Entities/DTOs:** `User.java`, `Role.java`, `LoginRequest.java`, `RegisterRequest.java`, `AuthResponse.java` | Controller (GRASP) — `AuthController` and all 9 controllers are pure routing delegates. They extract request data, call one service method, and return the result with zero business logic in the controller layer | Template Method (Behavioral) — `JwtAuthenticationFilter` extends Spring's `OncePerRequestFilter`. The parent defines the template (run exactly once per request); the subclass fills in the JWT-specific authentication step |
| **Member 4** | Loyalty Programme, Flight Tracking & AI Trip Suggestions + UML Diagrams | **Frontend:** `LoyaltyPage.jsx`, `TrackPage.jsx` **Controllers:** `LoyaltyController.java`, `FlightTrackingController.java`, `TripSuggestionController.java` **Services:** `LoyaltyService.java`, `FlightTrackingService.java`, `GeminiService.java` **Config:** `DataInitializer.java` **Factories:** `BookingFactory.java`, `DefaultBookingFactory.java` **Entities/DTOs:** `LoyaltyAccount.java`, `LoyaltyTransaction.java`, `LoyaltyTier.java`, `FlightTrackingResponse.java` **UML:** Use Case Diagram, Class Diagram, 4 Activity Diagrams, 4 State Diagrams | Information Expert (GRASP) — `LoyaltyAccount` owns `currentPoints`, `lifetimePoints`, and `tier`, and therefore owns all logic operating on that data — `addPoints()`, `redeemPoints()`, `recalculateTier()`. No external class computes loyalty state | — |
```
---
